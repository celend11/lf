...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private boolean handleException(final Throwable ex) {
        ...
        final StackTraceElement[] stacks = ex.getStackTrace();
        String message = ex.toString() + "<br/>";
        if (ex.getMessage() != null) {
            message = message + ex.getMessage() + "<br/>";
        }
        for (StackTraceElement stack : stacks) {
            message = message + stack.toString() + "<br/>";
        }
        // final String errorlog = message;
        ex.printStackTrace();
        // 使用Toast来显示异常信息
        showToast(mContext, "很抱歉，程序出现异常，即将退出.") ; // ，应用将提交这个错误报告，我们会尽快修复
        try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
