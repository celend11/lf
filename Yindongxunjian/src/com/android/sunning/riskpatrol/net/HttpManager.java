...
/**
 * Created by sunning on 14-09-28. 真实实现请求和响应的相关代码
 */
public final class HttpManager {
	 ...
	 * @param handler
	 */
	public static void request(final RequestParams params,
			final HandlerManager.SunnyHandler handler, String apiName) {
				HTTP_UTILS.send(HttpRequest.HttpMethod.POST,Const.URL+apiName,
				params, new RequestCallBack<String>() {
					Message msg = handler.obtainMessage();

					@Override
					public void onSuccess(
							ResponseInfo<String> stringResponseInfo) {
					...
					public void onFailure(HttpException httpException,
						...
						msg.what = Const.FAIL;
						BException sunnyException = new BException(
	 ...
	 * @param handler
	 */
	public static void requestByGet(final RequestParams params,
			final HandlerManager.SunnyHandler handler, String apiName) {
		if (params != null) {
			sing(params);
		}
		...
		HTTP_UTILS.send(HttpRequest.HttpMethod.GET, reqParams, null,
	 ...
	 * @param params
	 */
	public static void imgUpload(RequestParams params,
			final HandlerManager.SunnyHandler handler) {
		if (params != null) {
			sing(params);
		}
		LogUtils.e(params + "@@@@");
		// String reqParams = getParams(params ,
		// Const.InterfaceName.CHANGE_HEADER_IMG) ;
		httpHandler = HTTP_UTILS.send(HttpRequest.HttpMethod.POST, Const.URL
				+ Const.InterfaceName.UPLOAD_ATTACH, params,
				new RequestCallBack<Object>() {
					@Override
					public void onSuccess(
							ResponseInfo<Object> objectResponseInfo) {
						Message msg = handler.obtainMessage();
						if (objectResponseInfo != null
								&& objectResponseInfo.result != null) {
							try {
								JSONObject json = new JSONObject(
										objectResponseInfo.result.toString());
						...
						LogUtils.e("onSuccess"
								+ objectResponseInfo.reasonPhrase.toString());
					}

					@Override
					public void onFailure(HttpException e, String s) {
						BException exception = new BException(s, e
								.getExceptionCode());
						handler.sendMessage(handler.obtainMessage(
								Const.UPLOAD_FAIL, 0, 0, exception));
						LogUtils.e("上传失败了，但是我也不知道因为神马～！～！～！～onFailure" + s
								+ "=======" + e.toString());
					}
	...
	private static String getParams(RequestParams params, String apiName) {
		...
		for (NameValuePair paramsNV : list) {
			sBuffer.append(paramsNV.getName()).append("=")
					.append(paramsNV.getValue()).append("&");
	...
	private static void sendMessage(int what, Object data,
			HandlerManager.SunnyHandler handler) {
		Message msg = handler.obtainMessage();
		msg.what = what;
		msg.obj = data;
		handler.sendMessage(msg);
